...
public class Project {
    ...
    public static long FCFS(File file) throws InterruptedException, IOException {
        int jobCount = 0;
        System.out.println("RUNNING FIRST COME FIRST SERVED");
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        for(int i = 0; i < testList.size(); i++){
           writer.write("FIRST COME FIRST SERVED TRIALS (n=" + n +")\n");
            for(int j = 0; j < 20; j++){
                long time = FCFS(testList.get(i));
                totalAvgTime += time;
        ...
        totalAvgTime = 0;
        for(int i = 0; i < testList.size(); i++){
            writer.write("ROUND ROBIN 2NS SLICE TRIALS (n=" + n +")\n");
            for(int j = 0; j < 20; j++){
                long time = RoundRobin(testList.get(i), 2);
                totalAvgTime += time;
                writer.write("Trial " + (j+1) + " Average Turnaround Time: " + time + "ns\n");
            }
            writer.write("n=" + n +" Average Turnaround Time: " + totalAvgTime/20 + "\n");
